...
def clean_text_column(series: pd.Series,
    ...
    """
    cleaned_text = series.fillna('')

    cleaned_text = (cleaned_text.astype(str)
    ...
    if remove_nan:
        cleaned_text = cleaned_text.replace({'nan': '', '': None}).fillna('')
...
def main():
    ...
    df_filtered = df_filtered[df_filtered['name'].str.len() > 0]
    df_filtered = df_filtered[df_filtered['key'].str.len() > 0]
    ...
    df_haki = pd.read_csv("data/dataframes/df_haki_table.csv")
    ...
    df_cleaned.to_csv("data/dataframes/df_final_onepiece.csv", index=False)
