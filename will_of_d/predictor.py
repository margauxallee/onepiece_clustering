...
# Drop rows where affiliations, residence and height are all empty
df_characters = df_characters[~(
    df_characters['affiliations'].isna() & 
    df_characters['residence'].isna() & 
    df_characters['height'].isna()
)].copy()
...
affiliation_counts = df_characters['affiliations'].explode().value_counts()
frequent_affiliations = affiliation_counts[affiliation_counts >= 3].index
affiliations_dummies = pd.get_dummies(df_characters['affiliations'].explode())[frequent_affiliations].groupby(level=0).sum()
